import numpy as np 
from .utils import Skew, RightNull, Homogenize
# import sympy
...
def SimpleTriangulation(x1, x2, P1, P2):
    ...
    '''
    x1, x2 = x1.reshape(2,1), x2.reshape(2,1)
    if x1.shape == (2,1):
    ...
    check if P1 is [I | 0]
    '''
    if np.allclose(P1[:, -1], np.zeros((3,1))):
        pseudoP1 = P1.T
        C1 = np.zeros((4,1))
        C1[-1, 0] = 1
    else:
    ...
    X_intersect = np.multiply(X_inf, piC1) - np.multiply(C1, piX_inf)
